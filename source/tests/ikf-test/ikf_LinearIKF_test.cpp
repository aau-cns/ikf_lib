...
public:
  SimInstance(const Eigen::MatrixXd &F, const Eigen::MatrixXd &G, const Eigen::MatrixXd &Q,
              ...
              std::shared_ptr<ikf::IsolatedKalmanFilterHandler> ptr_Handler) : ID(ID), dt(dt), std_dev_p(std_dev_p), std_dev_a(std_dev_a), std_dev_p_rel(std_dev_p_rel), ptr_IKF(new ikf::SimpleLinearIKF(ptr_Handler, "name" + std::to_string(ID), ID)) {
...
public:
  ...
  std::shared_ptr<ikf::SimpleLinearIKF> ptr_IKF;
  Trajectory traj;
  Trajectory traj_est;

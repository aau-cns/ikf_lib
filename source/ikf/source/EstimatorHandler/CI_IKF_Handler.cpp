...
size_t CI_IKF_Handler::get_propagation_sensor_ID(const size_t ID) {
  ...
  if (lock.try_lock()) {
    ...
    ikf::Logger::ikf_logger()->error("CI_IKF_Handler::get_propagation_sensor_ID(): mutex FAILED");
    return 0;
  }
}
...
std::string CI_IKF_Handler::get_type_by_ID(const size_t ID) {
  ...
  if (lock.try_lock()) {
    ...
    ikf::Logger::ikf_logger()->error("CI_IKF_Handler::get_type_by_ID(): mutex FAILED");
    return std::string("");
  }
}
...
bool CI_IKF_Handler::get_belief_at_t(const size_t ID, const Timestamp &t, pBelief_t &bel,
  ...
  if (lock.try_lock()) {
    if (exists(ID)) {
      ...
      ikf::Logger::ikf_logger()->warn("CI_IKF_Handler::get_belief_at_t() from non-local estimator ["
                                      + std::to_string(ID) + "] using the agent handler");
      bool res = m_pAgentHandler->get_belief_at_t(ID, t, bel, type);
      if (!res) {
        ikf::Logger::ikf_logger()->error("CI_IKF_Handler::get_belief_at_t() from non-local estimator ["
                                         + std::to_string(ID) + "] failed");
      }
      return res;
    }
  } else {
    ikf::Logger::ikf_logger()->error("CI_IKF_Handler::get_belief_at_t(): mutex FAILED");
    return false;
  }
}
  ...
  if (lock.try_lock()) {
    ...
    if (remote_IDs.size()) {
      if (!m_pAgentHandler->get_beliefs_at_t(remote_IDs, remote_types, t, remote_beliefs)) {
        res = false;
        ikf::Logger::ikf_logger()->error("CI_IKF_Handler::get_beliefs_at_t() from non-local estimators failed");
      }
    }
    ...
    ikf::Logger::ikf_logger()->error("CI_IKF_Handler::get_beliefs_at_t(): mutex FAILED");
    return false;
  }
}
...
ApplyObsResult_t CI_IKF_Handler::apply_observation(const Eigen::MatrixXd &R, const Eigen::VectorXd &z,
  ...
  if (lock.try_lock()) {
    ...
    ikf::Logger::ikf_logger()->info(
      "CI_IKF_Handler::apply_observation():  num local IDs=" + std::to_string(local_IDs.size())
    ...
    if (ID_agents.size() == 1) {
      ...
      ikf::Logger::ikf_logger()->debug("CI_IKF_Handler::apply_corrections_at_t(): apply_inter_agent_observation...");

      return apply_inter_agent_observation(R, z, t, h, IDs, cfg, remote_IDs, local_IDs);
    }
    return ApplyObsResult_t(eMeasStatus::DISCARED);
  } else {
    ikf::Logger::ikf_logger()->error("CI_IKF_Handler::apply_observation(): mutex FAILED");
...
std::set<IMultiAgentHandler::IDAgent_t> CI_IKF_Handler::IDs_to_Agent_IDs(const std::set<size_t> &IDs) {
  ...
  if (lock.try_lock()) {
    ...
    ikf::Logger::ikf_logger()->error("CI_IKF_Handler::IDs_to_Agent_IDs(): mutex FAILED");
  }
  return ID_agents;
}
...
std::map<size_t, pBelief_t> CI_IKF_Handler::get_dict_bel(const std::vector<size_t> &IDs, const Timestamp &t) {
  ...
  if (lock.try_lock()) {
    ...
    ikf::Logger::ikf_logger()->error("CI_IKF_Handler::get_dict_bel(): mutex FAILED");
  }
  return dict_bel;
}
...
bool CI_IKF_Handler::get_local_beliefs_and_FCC_at_t(
  ...
  if (lock.try_lock()) {
    ...
    for (size_t idx = 0; idx < IDs.size(); idx++) {
      ...
      if (exists(ID_I)) {
        ...
        if (get(ID_I)->get_belief_at_t(t, pBel, type)) {
          ...
          ikf::Logger::ikf_logger()->error(
            "CI_IKF_Handler::get_local_beliefs_and_FCC_at_t: could not get_belief_at_t for ID=[{:}]", ID_I);
          return false;
        }
      } else {
        ikf::Logger::ikf_logger()->error("CI_IKF_Handler::get_local_beliefs_and_FCC_at_t: unknown local ID=[{:}]",
                                         ID_I);
        return false;
      }
    }  // for-loop local ids:
    ...
    ikf::Logger::ikf_logger()->error("CI_IKF_Handler::get_local_beliefs_and_FCC_at_t(): mutex FAILED");
    return false;
  }
}
...
Eigen::MatrixXd CI_IKF_Handler::stack_Sigma_locally(const std::map<size_t, pBelief_t> &dict_bel, const Timestamp &t,
  ...
  RTV_EXPECT_TRUE_THROW(dict_FFC.size() == dict_bel.size(),
                        "CI_IKF_Handler::stack_Sigma_locally: not enough rows in FFCs");
  ...
  for (auto const &e_i : dict_bel) {
    ...
    RTV_EXPECT_TRUE_THROW(dict_FFC[id_row].size() >= dict_bel.size() - 1,
                          "CI_IKF_Handler::stack_Sigma_locally: not enough cols in FFCs");
